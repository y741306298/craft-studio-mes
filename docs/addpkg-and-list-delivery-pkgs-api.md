...
| `deliveryManId` | string | 条件必填 | 发货人 ID。普通非聚单平台、非 `CUSTOM` 订单缺少该字段时中断打包并返回错误。 |
| `deliverySiidId` | string | 条件必填 | 电子面单账号/打印配置 ID。普通非聚单平台、非 `CUSTOM` 订单缺少该字段时中断打包并返回错误。 |
...
对于普通快递 100 流程，请求格式合法但电子面单下单或预下单复打失败时，服务会中断打包并返回具体错误原因，不新增 `DeliveryPkg`、不更新零件打包数量，也不会降级生成本地 `CUSTOM` 面单。
...
仅当零件物流预设类型明确为 `CUSTOM` 时使用本地 `CUSTOM` 流程。非 `CUSTOM` 订单缺少 `deliveryManId`、`deliverySiidId` 或按承运商与工厂查不到快递 100 Token 时，会中断打包并返回对应错误，不再自动降级。
...
- 若没有可用预下单记录，则调用快递 100 下单打印；成功后创建包裹并保存物流单号，失败时中断打包并返回错误原因，不创建包裹、不更新零件打包数量。
- 预下单面单缺少打印设备、复打异常或复打返回失败时，同样中断打包并返回错误原因；复打失败的 `DeliveryRecord` 保留为 `PRINT_FAILED`，不再降级为自定义面单。
